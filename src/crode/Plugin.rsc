 ...
 * After calling this function from the terminal, all files with extension .crode will be parsed using the parser defined in module crode::Parser.
...
int main() {
  ...
  registerLanguage(
    language(
      pathConfig(srcs=[|project://creative-coding-dsl/src|]),
      "Canvas",      // name of the language
      {"crode"},            // extension, e.g., example.lbr
      "crode::Server",      // module to import, this one
      "contributions"
    )
  );
  return 0;
 ...
 */
void clearCrode() {
  unregisterLanguage("Canvas", {"crode"});
}

/*
 * Use this function to run your tests. It will show how many tests sucrodeeeded.
 * Note that an invalid test "succeeds" when the check fails
 */
void runTests() {
  fails = 0;
  validFiles = |project://creative-coding-dsl/src/crode/test/valid|.ls;
  invalidFiles = |project://creative-coding-dsl/src/crode/test/invalid|.ls;
